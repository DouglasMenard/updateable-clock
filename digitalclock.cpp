...
void printMenu(const char* options[], unsigned int numStrings, unsigned char width) {
    cout << nCharString(width, '*') << endl;
    for (int i = 0; i < numStrings; i++) {
        int spacing = width - (6 + strlen(options[i])) - 1;
        cout << "*" << " " << i + 1 << " - " << options[i] << nCharString(spacing, ' ') << "*" << endl;
        if (i == numStrings - 1) {
            break;
        }
        cout << endl;
